package top.philsongzi.mydb.backend;

import org.apache.commons.cli.*;
import top.philsongzi.mydb.backend.dm.DataManager;
import top.philsongzi.mydb.backend.server.Server;
...
import top.philsongzi.mydb.backend.vm.VersionManager;
import top.philsongzi.mydb.backend.vm.VersionManagerImpl;
import top.philsongzi.mydb.common.Error;

/**
 * 服务器的启动入口，启动一个 Server 实例，监听端口，等待客户端连接。
 * 这个类解析了命令行参数。很重要的参数就是 -open 或者 -create。Launcher 根据两个参数，来决定是创建数据库文件，还是启动一个已有的数据库。
 * @author 小子松
 * @since 2023/8/3
 */
